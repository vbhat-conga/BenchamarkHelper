    ...
    public static class Utils
    {
            ...
            public static KustoConnectionStringBuilder GenerateConnectionString(string clusterUrl, AuthenticationModeOptions authenticationMode, string tenantId)
            {
                ...
                switch (authenticationMode)
                {
                    case AuthenticationModeOptions.UserPrompt:
                        ...
                        //return new KustoConnectionStringBuilder(clusterUrl).WithAadSystemManagedIdentity();
        ...
        public static class Queries
        {
            ...
            public static ClientRequestProperties CreateClientRequestProperties(string scope, string timeout = null)
            {
                ...
                {
                    Application = "RedisBenchMarkHelper.csproj",
            ...
            public static async Task ExecuteAsync(IDisposable client, string configDatabaseName, string command)
            {
                try
                {
                    ...
                    //AnsiConsole.WriteLine($"Response from executed command '{command}':\n--------------------\n");
        ...
        public static class Ingestion
        {
            ...
            {
                AnsiConsole.WriteLine($"Sleeping {WaitForIngestSeconds} seconds for queued ingestion to complete. Note: This may take longer depending on the file size and ingestion batching policy.");
                ...
                {
                    // Define tasks
                    var task1 = ctx.AddTask("Inserting data");
                    for (var i = WaitForIngestSeconds * 4; i >= 0; i--)
                    {
        ...
        {
            AnsiConsole.MarkupLine($"[red]Script failed with error: {error}[/]\n");
            if (e != null)
            {
